...
const createTouchTexture = () => {
  ...
  return {
    ...
    addTouch,
    update,
    // Accessors for radiusScale
    setRadiusScale: (v) => { radius = 0.1 * size * v; }
  };
};

const createLiquidEffect = (texture, opts) => {
  const strength = opts?.strength ?? 0.025;
  const freq = opts?.freq ?? 4.5;
  const fragment = `
...
const setup = () => {
  ...
  if (mustReinit) {
    ...
    if (props.liquid) {
      ...
      touch.setRadiusScale(props.liquidRadius);
    ...
    if (props.noiseAmount > 0) {
      ...
      if (composer && composer.passes.length > 0) {
        composer.passes.forEach(p => {
          if (p.renderToScreen !== undefined) p.renderToScreen = false;
        });
      }
    ...
    if (t.touch) {
      t.touch.setRadiusScale(props.liquidRadius);
    }
  }
